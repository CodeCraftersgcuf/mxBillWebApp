...
const Profile = () => {
  ...
  useEffect(() => {
    ...
    const storedProfilePicture =
      contextProfilePicture || Cookies.get("profilePicture");
    const storedPhone = Cookies.get("phoneNumber");
    const storedDob = Cookies.get("dob");
    const storedOccupation = Cookies.get("occupation");
  ...
  const { mutate: updateProfileMutation, isLoading } = useMutation({
    ...
    onSuccess: (response) => {
      ...
      Cookies.set("profilePicture", fullProfilePictureURL || "", {
        secure: true,
        sameSite: "strict",
      });
      Cookies.set("phoneNumber", updatedProfile?.phone || "", {
        secure: true,
        sameSite: "strict",
      });
      Cookies.set("dob", updatedProfile?.dob || "", {
        secure: true,
        sameSite: "strict",
      });
      Cookies.set("gender", updatedProfile?.gender || "", {
        secure: true,
        sameSite: "strict",
      });
      Cookies.set("occupation", updatedProfile?.occupation || "", {
        secure: true,
        sameSite: "strict",
      });
      //   Cookies.set("
